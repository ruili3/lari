...
def rescale_image_ldi(image, ldi, camera_intrinsics, output_resolution):
    """ Jointly rescale a (image, ldi) 
        so that (out_width, out_height) >= output_res
    """
    image = ImageList(image)
    input_resolution = np.array(image.size)  # (W,H)
    output_resolution = np.array(output_resolution)
    if ldi is not None:
        # can also use this with masks instead of ldi
        assert tuple(ldi.shape[:2]) == image.size[::-1]

    # define output resolution
    assert output_resolution.shape == (2,)
    scale_final = max(output_resolution / image.size) + 1e-8

    # align the shortest side with target resolution
    output_resolution = np.floor(input_resolution * scale_final).astype(int)

    # first rescale the image so that it contains the crop
    image = image.resize(tuple(output_resolution), resample=lanczos if scale_final < 1 else bicubic)
    if ldi is not None:
        ldi = cv2.resize(ldi, output_resolution, fx=scale_final,
                              fy=scale_final, interpolation=cv2.INTER_NEAREST)
    ...
    return image.to_pil(), ldi, camera_intrinsics

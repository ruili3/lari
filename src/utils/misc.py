...
def invalid_to_zeros(arr, valid_mask, ndim=999):
    '''
    flatten the tensor to dimension == ndim, with the last dim (3 point cloud dim) preserved.
    if the input is (b h w l 3), the output is (b hwl 3) if ndim==3
    ...
    def update(self, **kwargs):
        for k, v in kwargs.items():
            ...
            if isinstance(v, torch.Tensor):
                v = v.item()
            if isinstance(v, tuple): v = v[0] # for evalulation metrics where the first output is error the second is count.
...
class BSAgonisticMetricLogger(MetricLogger):
    '''
...
class NativeScalerWithGradNormCount:
    '''
    computes the gradient norm and clip the gradient
    if specified.
    '''

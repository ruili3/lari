"""Blender script to render images of a 3D model (.glb/.obj/.fbx).

The object is normalized, then a set of images is rendered from cameras sampled
around it. Each view's RGB image and 3x4 camera RT matrix are saved to
<output_dir>/<seq_name>/<object_uid>/.

Example usage:
    blender -b -P zero123_blender_script.py -- \
...
def save_images(object_file: str) -> None:
    ...
    seq_name = os.path.dirname(object_file).split("/")[-1]
    ...
    for i in range(args.num_images):
        ...
        # render the image
        render_path = os.path.join(args.output_dir, seq_name, object_uid, f"{i:03d}.png")
        scene.render.filepath = render_path
        bpy.ops.render.render(write_still=True)

        # save camera RT matrix
        RT = get_3x4_RT_matrix_from_blender(camera)
        RT_path = os.path.join(args.output_dir, seq_name, object_uid, f"{i:03d}.npy")
        np.save(RT_path, RT)
